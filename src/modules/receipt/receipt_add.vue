	...
	<div class="attendance_add">
		<div>
			...
			<form class="form-base">
				...
				<div class="flex form-mb">
					<div class="form-left">报销金额</div>
					<div class="felx-1 form-right">
						<input name="days" type="text" placeholder="请填写金额" v-model="formData.money"/>
					</div>
				</div>
				<div class="flex">
...
<script>
	import { submitReceipt } from '../../service/receipt'
...
//				options : ['xxx.doc','xxx.xml']
		...
		mounted() {
			this.$root.$emit.apply(this.$root, ['change-header'].concat(["新增报销", true, true]));
		},
		methods:{
			addFile(){
				Toast("待开发")
